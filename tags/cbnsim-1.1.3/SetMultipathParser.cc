...
WorkloadEvent* SetMultipathParser :: parse_event( WorkloadParser& parser )
  throw ( parse_exception )
{
  parser.eat( WorkloadParser::ID ); // set_predicate
  unsigned int from_nid = parser.eat_integer();
  unsigned int to_nid = parser.eat_integer();
  double ts = parser.eat_double();
  unsigned int inte = parser.eat_integer();
  Predicate p;
  ...
  return new SetMultipathEvent( from_nid, to_nid, ts, p, inte );
}

void SetMultipathParser :: constraint_list( WorkloadParser& parser, Filter& filt )
  throw ( parse_exception )
{
