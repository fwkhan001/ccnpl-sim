...
//
// $Id: PublishContentParser.cc,v 1.2 2004/02/03 06:16:21 hallcp Exp $
//
...
WorkloadEvent* PublishContentParser :: parse_event( WorkloadParser& parser )
  throw ( parse_exception )
{
  parser.eat( WorkloadParser::ID ); // publish_content
  unsigned int nid = parser.eat_integer();
  double ts = parser.eat_double();
  unsigned int size = parser.eat_integer();
  unsigned int bytesize = parser.eat_integer();
  unsigned int popularity_class = parser.eat_integer();
  //cout << popularity_class << endl;
  unsigned int receiver_win = parser.eat_integer();
  unsigned int fec_perc = parser.eat_integer();
  Predicate p;
  ...
  return new PublishContentEvent( nid, ts, p, size, bytesize, receiver_win, fec_perc, popularity_class);
}

void PublishContentParser :: constraint_list( WorkloadParser& parser, Filter& filt )
  throw ( parse_exception )
{
